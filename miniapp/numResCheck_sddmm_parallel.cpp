...
int main(int argc, char *argv[])
{
    ...
    { /* distribute A,B and respect communication, setup sparse comm*/
        ...
        Aloc.data.resize(Aloc.m * Aloc.n);
        Bloc.data.resize(Bloc.m * Bloc.n);
        for(size_t i = 0; i < Aloc.m; ++i){
            idx_t gidx = Cloc.ltgR[i];
            for(size_t j = 0; j < Aloc.n; ++j) Aloc.at(i,j) = gidx /(real_t) 1000; 
        }
        for(size_t i = 0; i < Bloc.m; ++i){
            idx_t gidx = Cloc.ltgC[i];
            for(size_t j = 0; j < Bloc.n; ++j) Bloc.at(i,j) = gidx /(real_t) 1000; 
        }
        ...
        real_t allsum, mysum = std::accumulate(Cloc.ownedVals.begin(), Cloc.ownedVals.end(), 0.0);
        MPI_Reduce(&mysum, &allsum, 1, MPI_REAL_T, MPI_SUM, 0, cartcomm);
        if(rank == 0) cout << allsum << endl;
    }
    /* instance #2: dense */
    {
        for(auto& elm : Cloc.vv) elm = 0.0;
