...
namespace SpKernels {
    ...
    inline idx_t get_pid_from_indx(idx_t g_index, idx_t units_per_processor, idx_t units_remainder)
    {
            ...
            MPI_Comm world_comm)
    {
        ...
        idx_t rows_pp = M/size, rows_ppR = M%size;
        idx_t cols_pp = N/size, cols_ppR = M%size;
        /* for my local rows, assign them to their responsible owners */
        vector<vector<int>> sendBuff(size);
        for(size_t i = 0; i < Cloc.lrows;++i){
           idx_t gid = Cloc.ltgR[i]; 
           idx_t where_to_send = get_pid_from_indx(gid, rows_pp, rows_ppR); 
        }
    }
